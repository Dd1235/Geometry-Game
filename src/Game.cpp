...
void Game::init(const std::string &filePath)
{
    ...
    if (isFullscreen)
    {
        m_window.create(sf::VideoMode(width, height), "Geometry game",
                        sf::Style::Fullscreen);
    }
    else
    {
...
void Game::spawnPlayer()
{
    ...
    // Adding shape
    // FR FG FB are fill color RGB values
    // OR OG OB are outline color RGB values
    ...
    // we have made a new entity, since it is the player we keep a reference to it
    m_player = entity;
}
...
void Game::spawnEnemy()
{
    ...
    Vec2 vel(xVel, yVel);
    vel.normalize(); // make it a unit vector
    ...
    // Enemy score, is proportional to number of it's vertices, add score component
